...
constructor(
  ...
  // Lifecycle hook
  ngOnInit(): void {
    const businessId = 14;  // just to work
    this.fetchAllVehicleTypes(businessId);
  }
  ...
  fetchAllVehicleTypes(businessId: number): void {
    ...
    this.vehicleservice.getVehicleDetails(businessId, 2).subscribe((response: any) => {
    ...
    this.vehicleservice.getVehicleDetails(businessId, 1).subscribe((response: any) => {
      response.forEach((vehicle: any) => {
        if (vehicle.photo && vehicle.photo.fileContent) {
          vehicle.photoUrl = `data:${vehicle.photo.contentType};base64,${vehicle.photo.fileContent}`;
        }
